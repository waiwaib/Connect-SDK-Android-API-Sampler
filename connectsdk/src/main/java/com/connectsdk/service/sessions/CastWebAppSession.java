...
public class CastWebAppSession extends WebAppSession {
    private final GoogleCastService service;
    ...
    public CastWebAppSession(LaunchSession launchSession, DeviceService service) {
        ...
        this.service = (GoogleCastService) service;
    }
    ...
    public void sendMessage(String message, final ResponseListener<Object> listener) {
        ...
        Cast.CastApi.sendMessage(service.getApiClient(), castServiceChannel.getNamespace(), message).setResultCallback(result -> {
            if (result.isSuccess()) {
                Util.postSuccess(listener, null);
            }
            else {
