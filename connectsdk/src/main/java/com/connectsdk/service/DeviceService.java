 ...
 * ###Overview
 * <p>
 * From a high-level perspective, DeviceService completely abstracts the functionality of a
 * particular service/protocol (webOS TV, Netcast TV, Chromecast, Roku, DIAL, etc).
 * <p>
 * ###In Depth
 * <p>
 * DeviceService is an abstract class that is meant to be extended. You shouldn't ever use
 * DeviceService directly, unless extending it to provide support for an additional service/protocol.
 * <p>
 * Immediately after discovery of a DeviceService, DiscoveryManager will set the DeviceService's
 * Listener to the ConnectableDevice that owns the DeviceService. You should not change the Listener
 * unless you intend to manage the lifecycle of that service. The DeviceService will proxy all
 * of its Listener method calls through the ConnectableDevice's ConnectableDeviceListener.
 * <p>
 * ####Connection & Pairing
 * <p>
 ...
 * ####Capabilities
 * <p>
 * All DeviceService objects have a group of capabilities. These capabilities can be implemented
 * by any object, and that object will be returned when you call the DeviceService's capability
 * methods (launcher, mediaPlayer, volumeControl, etc).
 * @noinspection unused
 */
public class DeviceService implements DeviceServiceReachabilityListener, ServiceCommandProcessor {
    ...
    /**
     * An array of capabilities supported by the DeviceService.
     * This array may change based off a number of factors.
     * <p>
    ...
    public static DeviceService getService(JSONObject json) {
        ...
        try {
            ...
            if (className.equalsIgnoreCase("GoogleCastService"))
                return null;

            newServiceClass = (Class<DeviceService>)
                    Class.forName(Objects.requireNonNull(DeviceService.class.getPackage()).getName() + "." + className);
    ...
    /**
     * Will attempt to connect to the DeviceService. The failure/success will be reported back to
     * the DeviceServiceListener. If the connection attempt reveals that pairing is required,
     * the DeviceServiceListener will also be notified in that event.
     */
    public void connect() {
    ...
    /**
     * Will attempt to disconnect from the DeviceService. The failure/success will
     * be reported back to the DeviceServiceListener.
     */
    ...
    /**
     * Explicitly cancels pairing in services that require pairing. In some services,
     * this will hide a prompt that is displaying on the device.
     */
    ...
    /**
     * Will attempt to pair with the DeviceService with the provided pairingData.
     * The failure/success will be reported back to the DeviceServiceListener.
     *
     * @param pairingKey Data to be used for pairing. The type of this parameter will vary
     *                  depending on what type of pairing is required, but is likely to
     *                   be a string (pin code, pairing key, etc).
     */
    ...
    /**
     * Test to see if the capabilities array contains at least one capability in a given set of
     * capabilities. See the individual Capability classes for acceptable capability values.
     * <p>
    ...
    public void addCapability(final String capability) {
        if (capability == null || capability.isEmpty() || this.mCapabilities.contains(capability))
            return;
        ...
        Util.runOnUI(() -> {
            ...
            if (listener != null)
                listener.onCapabilitiesUpdated(DeviceService.this, added, new ArrayList<>());
        });
    }
    ...
    public void addCapabilities(final List<String> capabilities) {
        ...
        for (String capability : capabilities) {
            if (capability == null || capability.isEmpty() || mCapabilities.contains(capability))
                continue;
        ...
        Util.runOnUI(() -> {
            if (listener != null)
                listener.onCapabilitiesUpdated(DeviceService.this, capabilities, new ArrayList<>());
        });
    }
    ...
    public void removeCapability(final String capability) {
        ...
        Util.runOnUI(() -> {
            ...
            if (listener != null)
                listener.onCapabilitiesUpdated(DeviceService.this, new ArrayList<>(), removed);
        });
    }
    ...
    public void removeCapabilities(final List<String> capabilities) {
        ...
        Util.runOnUI(() -> {
            if (listener != null)
                listener.onCapabilitiesUpdated(DeviceService.this, new ArrayList<>(), capabilities);
        });
    }
    ...
    public interface DeviceServiceListener {
        ...
        /**
         * If the DeviceService requires an active connection (websocket, pairing, etc)
        ...
        /**
         * After the connection has been successfully established, and after pairing
         * (if applicable), this method will be called.
         *
        ...
        /**
         * There are situations in which a DeviceService will update the capabilities it supports
         * and propagate these changes to the DeviceService. Such situations include:
         ...
         * @param error Error with a description of any errors causing the disconnect.
         *              If this value is nil, then the disconnect was clean/expected.
         */
         ...
         * @param pairingData Any data that might be required for the pairing process,
         *                    will usually be nil
         */
        void onPairingRequired(DeviceService service, PairingType pairingType, Object pairingData);

        /**
         * This method will be called upon pairing success. On pairing success,
         * a connection to the DeviceService will be attempted.
         *
