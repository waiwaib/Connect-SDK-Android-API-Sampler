...
public class FireTVService extends DeviceService implements MediaPlayer, MediaControl {
            ...
            handleAsyncFutureWithConversion(listener, asyncFuture,
                    data -> {
                        ...
                        if (metaJson.has(META_ICON_IMAGE)) {
                            icon = metaJson.getString(META_ICON_IMAGE);
                        }
                        return new MediaInfo.Builder(data.getSource(), metaJson.getString(META_MIME_TYPE))
                                .setTitle(metaJson.getString(META_TITLE))
                                .setDescription(metaJson.getString(META_DESCRIPTION))
                                .setIcon(icon)
                                .build();
                    }, error);
        } catch (Exception e) {
    ...
    public void getPlayState(final PlayStateListener listener) {
        ...
        try {
            ...
            handleAsyncFutureWithConversion(listener, asyncFuture,
                    this::createPlayStateStatusFromFireTVStatus, error);
        } catch (Exception e) {
            Util.postError(listener, new FireTVServiceError(error, e));
        }
    }
    ...
    private String getMetadata(MediaInfo mediaInfo)
        ...
        if (mediaInfo.getSubtitleInfo() != null) {
            JSONArray tracksArray = getTracksArray(mediaInfo);
    ...
    @NonNull
    private static JSONArray getTracksArray(MediaInfo mediaInfo) throws JSONException {
    ...
    private void setMediaSource(MediaInfo mediaInfo, final LaunchListener listener) {
        ...
        handleAsyncFutureWithConversion(listener, asyncFuture,
                data -> createMediaLaunchObject(), error);
    }
