...
public class FireTVDiscoveryProvider implements DiscoveryProvider {
    ...
    class FireTVDiscoveryListener implements DiscoveryController.IDiscoveryListener {
        ...
        private void updateServiceDescription(ServiceDescription serviceDescription, RemoteMediaPlayer remoteMediaPlayer) {
            try {
                // 获取实际类型 PlayerDeviceImpl
                Class<?> clazz = Class.forName("com.amazon.whisperplay.fling.media.controller.impl.PlayerDeviceImpl");
                // 使用反射获取 mWPDevice 字段
                Field mWPDeviceField = clazz.getDeclaredField("mWPDevice");
                mWPDeviceField.setAccessible(true); // 允许访问私有字段
                // 获取 remoteMediaPlayer 的实际类型
                Object actualPlayerDevice = clazz.cast(remoteMediaPlayer);
                // 获取 mWPDevice 实例
                Object mWPDevice = mWPDeviceField.get(actualPlayerDevice);
                // 确保 mWPDevice 是 Device 类型
                if (mWPDevice instanceof Device) {
                    Device device = (Device) mWPDevice;
                    if (!device.getRoutes().isEmpty()){
                        ...
                        if (route != null) {
                            ...
                            serviceDescription.setServiceFilter("FireTV");
                            serviceDescription.setManufacturer("Amazon");
                            serviceDescription.setPort(8080);
                            serviceDescription.setIpAddress(route.ipv4);
                        }
                    }
                }
            } catch (Exception e) {
                Log.e("FireTVDiscoveryProvider", "Error updating service description", e);
            }
        }
    }
}