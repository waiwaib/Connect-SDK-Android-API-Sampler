...
public class CastDiscoveryProvider implements DiscoveryProvider {
        ...
        if (mMediaRouteSelector == null) {
            try {
                ...
                .addControlCategory(CastMediaControlIntent.categoryForCast(
                        GoogleCastService.getApplicationID()))
                .build();
            } catch (IllegalArgumentException e) {
                Log.d(Util.T, "Invalid application ID: " + GoogleCastService.getApplicationID());
                for (DiscoveryProviderListener listener : serviceListeners) {
                    listener.onServiceDiscoveryFailed(this, new ServiceCommandError(0,
                            "Invalid application ID: " + GoogleCastService.getApplicationID(), null));
                }
                return;
            }
        }
    ...
    public void stop() {
        ...
        if (mMediaRouter != null) {
            Util.runOnUI(() -> mMediaRouter.removeCallback(mMediaRouterCallback));
        }
    }
    ...
    private class MediaRouterCallback extends MediaRouter.Callback {
        ...
        public void onRouteAdded(@NonNull MediaRouter router, @NonNull RouteInfo route) {
            ...
            if (castDevice!=null){
                ...
                if (isNew) {
                    String ip = castDevice.getInetAddress().getHostAddress();
                    foundService = new ServiceDescription(GoogleCastService.ID, uuid, ip);
                    ...
                    foundService.setServiceID(GoogleCastService.ID);
        ...
        @Override
        public void onRouteRemoved(@NonNull final MediaRouter router, @NonNull final RouteInfo route) {
            super.onRouteRemoved(router, route);

            CastDevice castDevice = CastDevice.getFromBundle(route.getExtras());
            assert castDevice != null;
            String uuid = castDevice.getDeviceId();
        ...
        @Override
        public void onRouteVolumeChanged(@NonNull MediaRouter router, RouteInfo route) {
