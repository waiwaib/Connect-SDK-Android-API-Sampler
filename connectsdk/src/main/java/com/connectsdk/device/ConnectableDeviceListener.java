 ...
 * <p>
 * It also serves as a proxy for message handling when connecting and pairing with each of
 * a ConnectableDevice's DeviceServices. Each of the DeviceService proxy methods are optional
 * and would only be useful in a few use cases.
...
public interface ConnectableDeviceListener {
     ...
     * have been connected and are ready to receive commands.
     *
    ...
    /**
     * When all of a ConnectableDevice's DeviceServices have become disconnected,
     * the disconnected message is sent.
     *
     ...
     * <p>
     * This method is called when a DeviceService tries to connect and finds out that
     * it requires pairing information from the user.
     *
    ...
    /**
     * When a ConnectableDevice finds & loses DeviceServices, that ConnectableDevice
     * will experience a change in its collective capabilities list. When such a change
     * occurs, this message will be sent with arrays of capabilities that were added & removed.
     * <p>
     * This message will allow you to decide when to stop/start interacting with a
     * ConnectableDevice, based off of its supported capabilities.
     *
